	...
	public void replacePassword(String user, String pass){
		for (int i = 0; i < userAndPass.length; i++) {
			if((userAndPass[i] != null) && ((userAndPass[i]).equals(user)) && (i % 2 == 0)) {
                ...
                System.out.println("Password replaced for user: " + userAndPass[i]);
                try {
        			writeToFile();
        		} catch (IOException e) {
        			e.printStackTrace();
        ...
        System.out.println("This user was not able to be found: " + user);
	}
	...
	public void writeToFile() throws IOException {
		...
		try {
			if(((userAndPass.length) % 2 == 0) && (userAndPass != null)) {
		        ...
		        System.out.println("The array length is mod 2 equals 0");
